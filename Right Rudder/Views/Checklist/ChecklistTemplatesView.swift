...
struct ChecklistTemplatesView: View {
    ...
    List {
      ForEach(Array(phaseGroups.enumerated()), id: \.element.id) { phaseIndex, phaseGroup in
        ...
        if (selectedCategory == "Instrument" || selectedCategory == "Commercial")
          && phaseIndex < phaseGroups.count - 1
        {
          spacingSection
        }
      }
  ...
  @ViewBuilder
  private func templateRowWithActions(
    template: ChecklistTemplate, index: Int, phaseGroup: PhaseGroup
  ) -> some View {
    if isEditing {
  ...
  private var phaseGroups: [PhaseGroup] {
    ...
    if selectedCategory == "Instrument" {
      ...
      let phaseOrder = [
        ...
        "Flight Reviews", "Instrument Rating", "Commercial Rating",
      ]
  ...
  private func moveTemplates(
    from source: IndexSet, to destination: Int, in templates: [ChecklistTemplate]
  ) {
