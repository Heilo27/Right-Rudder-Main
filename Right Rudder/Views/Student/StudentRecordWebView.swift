...
struct StudentRecordWebView: View {
  ...
  private func generateHTMLContentOptimized(currentDate: String) async -> String {
    ...
    let biographySection =
      student.biography?.isEmpty == false
      ? """
              <div class="info-item">
    ...
    let backgroundSection =
      student.backgroundNotes?.isEmpty == false
      ? """
              <div class="info-item">
    ...
    let checklistsSection =
      (student.checklistAssignments?.isEmpty ?? true) ? "" : generateChecklistsSection()

    let endorsementsSection =
      (student.endorsements?.isEmpty ?? true) ? "" : generateEndorsementsSection()
  ...
  private func generateChecklistsSection() -> String {
    ...
    let sortedAssignments = (student.checklistAssignments ?? []).sorted {
      assignment1, assignment2 in
    ...
    let checklistHTML = sortedAssignments.map { assignment in
      ...
      let itemsHTML = sortedItems.map { item in
        ...
        let completionDate =
          item.isComplete && item.completedAt != nil
      ...
      let commentsHTML =
        assignment.instructorComments?.isEmpty == false
        ? """
            <div class="instructor-comments">
  ...
  private func generateDocumentsSection() -> String {
    let documentsHTML = (student.documents ?? []).map { document in
      ...
      if document.fileData != nil {
        ...
        imageHTML =
          "<div style=\"text-align:center;color:#666;font-style:italic;margin:20px 0;padding:40px;border:2px dashed #ccc;border-radius:8px;\">📄 Document: \(document.filename)<br><small>Click to view full document</small></div>"
      } else {
        imageHTML =
          "<div style=\"text-align:center;color:#666;font-style:italic;margin:20px 0;\">No image data available</div>"
      }
