...
struct StudentDetailView: View {
  ...
  var body: some View {
    ...
    return
      ...
      .onAppear {
        ...
        Task {
          let studentUnlinked = await shareService.checkForStudentInitiatedUnlink(
            for: student, modelContext: modelContext)
          if studentUnlinked {
            await MainActor.run {
        ...
        Task {
          ...
          if hasActive {
            print(
              "🔄 Student has active share (accepted) - triggering automatic sync for: \(student.displayName)"
            )
            ...
            print(
              "⚠️ Student \(student.displayName) has share URL but hasn't accepted yet - skipping sync"
            )
          }
        }
      ...
      .onReceive(
        NotificationCenter.default.publisher(for: Notification.Name("checklistItemCompleted"))
      ) { _ in
      ...
      .alert("Delete Checklist", isPresented: $showingDeleteConfirmation) {
        ...
        if let checklist = checklistToDelete {
          Text(
            "Are you sure you want to delete '\(checklist.displayName)'? This action cannot be undone."
          )
        }
      }
      .alert("Unlink Student App", isPresented: $showingUnlinkConfirmation) {
        ...
        Text(
          "This will remove the student's access to their companion app. All shared data will be deleted from CloudKit. They will no longer be able to view their profile or upload documents. You can re-link them later if needed."
        )
      }
      .alert("Student Left Link", isPresented: $showStudentLeftNotification) {
        ...
        if let name = studentLeftName {
          Text(
            "\(name) has left the link. All assigned lessons have been removed from their app. You can send a new invite if needed."
          )
        } else {
          Text(
            "The student has left the link. All assigned lessons have been removed from their app.")
        }
      }
  }
  ...
  private func updateSortedLists() {
    print(
      "📊 Updating sorted lists - student has \(student.checklistAssignments?.count ?? 0) checklist assignment records"
    )
    ...
    if let assignments = student.checklistAssignments {
      ...
      for (index, assignment) in assignments.enumerated() {
        ...
        print(
          "📊   [\(index)] ID: \(assignment.id), Template: \(assignment.template?.name ?? "nil"), DisplayName: \(assignment.displayName), Progress: \(completedCount)/\(totalCount)"
        )
      }
    }
  ...
  private func extractLessonNumber(from templateName: String) -> Int? {
    ...
    guard let match = regex.firstMatch(in: templateName, range: range),
      let numberRange = Range(match.range(at: 1), in: templateName)
    else {
      return nil
    }
  ...
  private func sortChecklistProgress(_ progress: [ChecklistAssignment]) -> [ChecklistAssignment] {
    ...
    let sortedIncomplete = incompleteProgress.sorted { progress1, progress2 in
      ...
      return (progress1.template?.name ?? "").localizedCaseInsensitiveCompare(
        progress2.template?.name ?? "") == .orderedAscending
    }
  ...
  private func getPhasePriority(for templateName: String) -> Int {
    ...
    if templateName.contains("Student Onboard") || templateName.contains("Training Overview") {
      ...
      return 1  // Phase 1
    } else if templateName.contains("Pre-Solo") || templateName.contains("Solo")
      || templateName.contains("First Solo")
    {
      ...
      HStack(alignment: .top, spacing: 12) {
        VStack(alignment: .leading, spacing: 8) {
          ...
          VStack(alignment: .leading, spacing: 4) {
            ...
            Label(
              "Total Dual Given: \(String(format: "%.1f", student.totalDualGivenHours)) hours",
              systemImage: "clock"
            )
            .foregroundColor(.blue)
            .fontWeight(.medium)
        ...
        if let photoData = student.profilePhotoData {
          if let uiImage = UIImage(data: photoData),
            let optimizedImage = ImageOptimizationService.shared.optimizeImage(
              uiImage, maxSize: CGSize(width: 144, height: 144))
          {
      ...
      if student.endorsements?.isEmpty ?? true {
        ...
        LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 2), spacing: 12) {
          ForEach(Array(sortedEndorsements.enumerated()), id: \.element.id) { index, endorsement in
            VStack(spacing: 4) {
              EndorsementThumbnailView(endorsement: endorsement) {
