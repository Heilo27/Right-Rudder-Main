...
struct StudentsView: View {
  ...
  var body: some View {
      ...
      if allStudents.isEmpty {
        ...
        Task {
          ...
          if allStudents.isEmpty {
            ...
            if totalRecords > 0 {
              print(
                "💡 Found \(totalRecords) records in CloudKit - use Emergency Data Recovery in Settings"
              )
            }
          }
        }
        ...
        if hasDuplicates || hasEmptyNames {
          print(
            "⚠️ Detected corrupted students - duplicate IDs: \(hasDuplicates), empty names: \(hasEmptyNames)"
          )
    ...
    .onReceive(NotificationCenter.default.publisher(for: NSNotification.Name("StudentUpdated"))) {
      _ in
    ...
    do {
      ...
      if allStudents.isEmpty {
        ...
        for (index, student) in allStudents.enumerated() {
          print(
            "📊   [\(index)] \(student.displayName) - Active: \(!student.isInactive), ID: \(student.id)"
          )
        }
      }
    } catch {
  ...
  private func updateFilteredStudents() {
    let baseStudents =
      showingInactiveStudents
      ? allStudents.filter { $0.isInactive } : allStudents.filter { !$0.isInactive }

    print(
      "📊 Filtering students - Total: \(allStudents.count), Active: \(allStudents.filter { !$0.isInactive }.count), Showing inactive: \(showingInactiveStudents)"
    )
      ...
      cachedFilteredStudents = baseStudents.filter { student in
        let assignedCat = student.assignedCategory
          ...
          if assignedCat != nil || student.checklistAssignments?.isEmpty == false {
            print(
              "🔍 Filtering student '\(student.displayName)': assignedCategory='\(assignedCat ?? "nil")', filter='\(category)'"
            )
          }
        #endif
        ...
        if assignedCat == nil {
          ...
          #if DEBUG
            print(
              "🔍 Auto-detected category for '\(student.displayName)': '\(autoDetected ?? "nil")'")
          #endif
