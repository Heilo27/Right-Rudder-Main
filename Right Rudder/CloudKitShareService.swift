...
// MARK: - Library Models (for resolving student app library IDs)
struct LibraryChecklist: Codable, Identifiable {
    ...
    init(from decoder: Decoder) throws {
        ...
        self.templateIdentifier = try container.decodeIfPresent(String.self, forKey: .templateIdentifier)
    }
}
...
// MARK: - CloudKit Record Data Structures
...
struct StudentPersonalInfoRecord {
    ...
    static func fromCKRecord(_ record: CKRecord) -> StudentPersonalInfoRecord? {
        guard let studentId = record["studentId"] as? String,
              ...
              let lastModifiedBy = record["lastModifiedBy"] as? String else {
            return nil
        }
...
struct TrainingGoalsRecord {
    ...
    func toCKRecord(recordID: CKRecord.ID) -> CKRecord {
        let record = CKRecord(recordType: Self.recordType, recordID: recordID)
        record["studentId"] = studentId
        record["goalPPL"] = NSNumber(value: goalPPL)
        record["goalInstrument"] = NSNumber(value: goalInstrument)
        record["goalCommercial"] = NSNumber(value: goalCommercial)
        record["goalCFI"] = NSNumber(value: goalCFI)
        record["pplGroundSchoolCompleted"] = NSNumber(value: pplGroundSchoolCompleted)
        record["pplWrittenTestCompleted"] = NSNumber(value: pplWrittenTestCompleted)
        record["instrumentGroundSchoolCompleted"] = NSNumber(value: instrumentGroundSchoolCompleted)
        record["instrumentWrittenTestCompleted"] = NSNumber(value: instrumentWrittenTestCompleted)
        record["commercialGroundSchoolCompleted"] = NSNumber(value: commercialGroundSchoolCompleted)
        record["commercialWrittenTestCompleted"] = NSNumber(value: commercialWrittenTestCompleted)
        record["cfiGroundSchoolCompleted"] = NSNumber(value: cfiGroundSchoolCompleted)
        record["cfiWrittenTestCompleted"] = NSNumber(value: cfiWrittenTestCompleted)
        record["lastModified"] = lastModified
        return record
    }
    ...
    static func fromCKRecord(_ record: CKRecord) -> TrainingGoalsRecord? {
        ...
        return TrainingGoalsRecord(
            studentId: studentId,
            goalPPL: (record["goalPPL"] as? NSNumber)?.boolValue ?? false,
            goalInstrument: (record["goalInstrument"] as? NSNumber)?.boolValue ?? false,
            goalCommercial: (record["goalCommercial"] as? NSNumber)?.boolValue ?? false,
            goalCFI: (record["goalCFI"] as? NSNumber)?.boolValue ?? false,
            pplGroundSchoolCompleted: (record["pplGroundSchoolCompleted"] as? NSNumber)?.boolValue ?? false,
            pplWrittenTestCompleted: (record["pplWrittenTestCompleted"] as? NSNumber)?.boolValue ?? false,
            instrumentGroundSchoolCompleted: (record["instrumentGroundSchoolCompleted"] as? NSNumber)?.boolValue ?? false,
            instrumentWrittenTestCompleted: (record["instrumentWrittenTestCompleted"] as? NSNumber)?.boolValue ?? false,
            commercialGroundSchoolCompleted: (record["commercialGroundSchoolCompleted"] as? NSNumber)?.boolValue ?? false,
            commercialWrittenTestCompleted: (record["commercialWrittenTestCompleted"] as? NSNumber)?.boolValue ?? false,
            cfiGroundSchoolCompleted: (record["cfiGroundSchoolCompleted"] as? NSNumber)?.boolValue ?? false,
            cfiWrittenTestCompleted: (record["cfiWrittenTestCompleted"] as? NSNumber)?.boolValue ?? false,
            lastModified: lastModified
        )
    }
}
...
class CloudKitShareService: ObservableObject {
        ...
        do {
            ...
            do {
                ...
                if error.localizedDescription.contains("not deployed") || 
                   error.localizedDescription.contains("Production") ||
                    ...
                    print("   ✅ Schemas must be deployed to Production before publishing (see CLOUDKIT_SCHEMA_DEPLOYMENT.md)")
                    print("   ✅ After deployment, unlimited records can be created automatically")
                } else {
                    // Could be Development or Production - check error code
                    print("🌍 CloudKit Environment: Unable to determine (error: \(error.localizedDescription))")
    ...
    private func loadStudentLibrary() -> ChecklistLibrary? {
        ...
        guard let url = Bundle.main.url(forResource: "DefaultChecklistLibrary", withExtension: "json", subdirectory: "ChecklistLibrary") ??
            ...
            print("   Searched in: ChecklistLibrary/DefaultChecklistLibrary.json and DefaultChecklistLibrary.json")
            return nil
        }
        ...
        do {
            ...
            print("✅ Loaded student library with \(library.checklists.count) checklists from: \(url.lastPathComponent)")
            return library
        } catch {
    ...
    private func resolveLibraryTemplateID(templateIdentifier: String?) -> UUID? {
        ...
        if let checklist = library.checklists.first(where: { $0.templateIdentifier?.lowercased() == templateIdentifier.lowercased() }) {
            return checklist.id
        }
    ...
    private func resolveLibraryItemID(templateIdentifier: String?, itemOrder: Int) -> UUID? {
        ...
        guard let checklist = library.checklists.first(where: { $0.templateIdentifier?.lowercased() == templateIdentifier.lowercased() }) else {
            print("⚠️ resolveLibraryItemID: Could not find checklist with templateIdentifier: \(templateIdentifier)")
            return nil
        }
        ...
        if arrayIndex >= 0 && arrayIndex < sortedItems.count {
            ...
            print("⚠️ resolveLibraryItemID: Item order \(itemOrder) (array index \(arrayIndex)) is out of bounds for checklist '\(templateIdentifier)' (has \(sortedItems.count) items, order range: \(minOrder)-\(maxOrder), \(isOneBased ? "1-based" : "0-based"))")
            return nil
        }
    }
    ...
    private func resolveLibraryItemID(for itemProgress: ItemProgress, assignment: ChecklistAssignment) -> UUID? {
        // For custom checklists, use the instructor's ID (no library to resolve)
        guard !assignment.isCustomChecklist, let templateIdentifier = assignment.templateIdentifier else {
            return itemProgress.templateItemId
        }
        
        // Get the template to find the item's order
        guard let template = assignment.template,
              let templateItems = template.items?.sorted(by: { $0.order < $1.order }) else {
            print("⚠️ resolveLibraryItemID: Could not get template items for assignment: \(assignment.templateIdentifier ?? "unknown")")
            return itemProgress.templateItemId
        }
        
        // Find the template item that matches this ItemProgress
        guard let templateItem = templateItems.first(where: { $0.id == itemProgress.templateItemId }) else {
            print("⚠️ resolveLibraryItemID: Could not find template item for ItemProgress.templateItemId: \(itemProgress.templateItemId.uuidString.prefix(8))")
            print("   Available template item IDs: \(templateItems.prefix(5).map { $0.id.uuidString.prefix(8) }.joined(separator: ", "))")
            return itemProgress.templateItemId
        }
        
        // Resolve the library item ID using the item's order
        if let libraryItemID = resolveLibraryItemID(templateIdentifier: templateIdentifier, itemOrder: templateItem.order) {
    ...
    private func createChecklistAssignmentRecord(from assignment: ChecklistAssignment, student: Student) -> ChecklistAssignmentRecord {
        // Get template items to look up order values
        let templateItems = assignment.template?.items?.sorted { $0.order < $1.order } ?? []
        let templateItemOrderMap = Dictionary(uniqueKeysWithValues: templateItems.map { ($0.id, $0.order) })
        ...
        let itemProgressData = (assignment.itemProgress ?? []).map { progress in
            ...
            let libraryItemID = resolveLibraryItemID(for: progress, assignment: assignment) ?? progress.templateItemId
            
            // Log ID resolution for debugging (log all to see what's happening)
            let progressIndex = (assignment.itemProgress ?? []).firstIndex(where: { $0.id == progress.id }) ?? 0
            if libraryItemID != progress.templateItemId {
                if progressIndex < 10 || progressIndex % 10 == 0 {
                    print("🔄 createChecklistAssignmentRecord[\(progressIndex)]: Resolved library item ID: \(progress.templateItemId.uuidString.prefix(8)) -> \(libraryItemID.uuidString.prefix(8))")
                }
            } else {
                // Always log failures - these are important
                print("⚠️ createChecklistAssignmentRecord[\(progressIndex)]: Could not resolve library item ID, using instructor ID: \(progress.templateItemId.uuidString.prefix(8))")
            }
        ...
        let libraryTemplateID = resolveLibraryTemplateID(templateIdentifier: assignment.templateIdentifier) ?? assignment.templateId
    ...
    private func initializeCustomChecklistDefinitionSchema() async {
        ...
        if environment == .production {
            print("ℹ️ Production mode detected - assuming CustomChecklistDefinition schema is already deployed")
        ...
        do {
            ...
            do {
                ...
                if let firstSharedZone = allSharedZones.first {
                    ...
                    let sharedDummyRecordID = CKRecord.ID(recordName: UUID().uuidString, zoneID: firstSharedZone.zoneID)
                    let sharedSchemaRecord = CKRecord(recordType: "CustomChecklistDefinition", recordID: sharedDummyRecordID)
                    ...
                    print("✅ CustomChecklistDefinition schema initialized in shared database (zone: \(firstSharedZone.zoneID.zoneName))")
                    ...
                    print("ℹ️ No shared zones exist yet - CustomChecklistDefinition schema will be available once shares are created")
                    print("   Schema initialized in private database should propagate to shared database automatically in Development")
                ...
                if saveError.code == .unknownItem {
                    ...
                    do {
                        ...
                        let defaultSchemaRecord = CKRecord(recordType: "CustomChecklistDefinition", recordID: defaultZoneDummyRecordID)
                        ...
                        print("✅ CustomChecklistDefinition schema initialized in shared database (default zone)")
                        ...
                        print("ℹ️ CustomChecklistDefinition schema will be available in shared database once shares are created")
                        print("   Schema from private database should propagate automatically in Development")
                        schemaInitialized = true  // Consider it initialized since private DB initialization succeeded
                    }
                } else if saveError.localizedDescription.contains("not deployed") || saveError.localizedDescription.contains("Production") {
    ...
    private func initializeItemProgressSchema() async {
        ...
        do {
            ...
            do {
                ...
                print("ℹ️ ItemProgress schema is now available in all CloudKit databases (private, shared, public)")
                
            } catch let saveError as CKError {
                if saveError.code == .unknownItem {
                    // Schema might already exist, or record was deleted
                    print("✅ ItemProgress schema appears to exist")
                } else if saveError.localizedDescription.contains("Production") || saveError.localizedDescription.contains("deploy") {
                    ...
                    print("     - assignmentId (String, Indexed, Queryable) - CRITICAL for student app queries")
                    print("     - notes (String)")
    ...
    func getSharedZone(for student: Student) async throws -> CKRecordZone? {
        ...
        do {
            ...
            guard let shareRecord = try? await privateDatabase.record(for: shareRecordID) as? CKShare else {
                print("⚠️ Share record not found in private database for student \(student.displayName)")
                return nil
            }
            
            print("🔍 Found share record in private database, zone: \(shareRecord.recordID.zoneID.zoneName)")
            ...
            if shouldPrintDiagnostics {
                ...
                for (index, participant) in shareRecord.participants.enumerated() {
                    ...
                    print("     - User identity lookup info: \(participant.userIdentity.lookupInfo?.emailAddress ?? "no email")")
                }
            ...
            if let shareURL = shareRecord.url {
                do {
                    ...
                    if #available(iOS 16.0, *) {
                        rootRecordIDFromMetadata = shareMetadata.hierarchicalRootRecordID ?? CKRecord.ID(recordName: student.id.uuidString, zoneID: customZone.zoneID)
                    } else {
                    ...
                    do {
                        ...
                        if shouldPrintDiagnostics {
                            print("✅ Found shared zone via share metadata: \(sharedZoneID.zoneName), owner: \(sharedZoneID.ownerName)")
                        }
                        return sharedZone
                    } catch {
            ...
            if shouldPrintDiagnostics {
                print("   Found \(allSharedZonesForDiagnostic.count) zones in shared database for diagnostic search")
            }
            ...
            if !allSharedZonesForDiagnostic.isEmpty {
                for zone in allSharedZonesForDiagnostic {
                    ...
                    if let shareInShared = try? await sharedDatabase.record(for: shareRecordIDInShared) as? CKShare {
                        if shouldPrintDiagnostics {
                            print("✅ DIAGNOSTIC: Found share record in shared database zone: \(zone.zoneID.zoneName)")
                        ...
                        let rootRecordIDFromShare = CKRecord.ID(recordName: student.id.uuidString, zoneID: zone.zoneID)
                        do {
                            ...
                            if shouldPrintDiagnostics {
                                print("   ⚠️ DIAGNOSTIC: Could not fetch root record from shared DB: \(error.localizedDescription)")
                            }
                        }
                    }
            ...
            if allSharedZonesForDiagnostic.isEmpty {
                if shouldPrintDiagnostics {
                    print("   ⚠️ DIAGNOSTIC: No zones found in allRecordZones() - zone may not be propagated yet")
                    print("   💡 This is normal immediately after share acceptance - CloudKit may take a few seconds")
                }
            } else {
                // Try to find student record in the zones we found
                if shouldPrintDiagnostics {
                    print("🔍 DIAGNOSTIC: Attempting to access root record (student record) directly in shared database...")
                }
                for zone in allSharedZonesForDiagnostic {
                    do {
                        let rootRecordIDInShared = CKRecord.ID(recordName: student.id.uuidString, zoneID: zone.zoneID)
                        let rootRecordInShared = try await sharedDatabase.record(for: rootRecordIDInShared)
                        if shouldPrintDiagnostics {
                            print("✅ DIAGNOSTIC: Found root record directly in shared zone: \(zone.zoneID.zoneName)")
                            print("   Zone owner: \(zone.zoneID.ownerName)")
            ...
            let directZoneID1 = CKRecordZone.ID(zoneName: customZoneName, ownerName: customZone.zoneID.ownerName)
            do {
            ...
            let directZoneID2 = CKRecordZone.ID(zoneName: customZoneName, ownerName: CKCurrentUserDefaultName)
            do {
            ...
            do {
                ...
                let directZoneID3 = CKRecordZone.ID(zoneName: customZoneName, ownerName: userRecordID.recordName)
            ...
            print("   Available zones: \(allSharedZones.map { "\($0.zoneID.zoneName) (owner: \($0.zoneID.ownerName))" })")
    ...
    func createShareForStudent(_ student: Student, modelContext: ModelContext) async -> URL? {
        ...
        do {
            ...
            let personalInfoRecordID = CKRecord.ID(recordName: "\(student.id.uuidString)-personal-info", zoneID: customZone.zoneID)
            ...
            let trainingGoalsRecordID = CKRecord.ID(recordName: "\(student.id.uuidString)-training-goals", zoneID: customZone.zoneID)
            ...
            do {
                ...
                if error.code == .serverRecordChanged {
                    // Record already exists - this is fine, just update it
                    do {
                        _ = try await saveRecordWithConflictResolution(trainingGoalsCKRecord, database: database)
            ...
            print("🔄 Creating all student records in private zone (will be available in shared zone once accepted)...")
            ...
            for assignment in assignments {
                do {
                    ...
                    createdCount += 1
                    print("   ✅ Created assignment \(createdCount)/\(assignments.count): \(assignment.templateIdentifier ?? "unknown") (with all ItemProgress)")
                    ...
                    print("   ⚠️ Failed to create assignment \(assignment.templateIdentifier ?? "unknown"): \(error)")
                }
            }
            
            print("✅ Created \(createdCount)/\(assignments.count) assignments in private zone")
            print("   All records are now ready and will be available in shared zone once student accepts the share")
    ...
    private func deleteAllSharedZoneData(for student: Student) async throws {
        ...
        do {
            ...
            let assignmentQuery = CKQuery(recordType: "ChecklistAssignment", predicate: assignmentPredicate)
            let assignmentResults = try await sharedDatabase.records(matching: assignmentQuery, inZoneWith: sharedZone.zoneID)
            ...
            if !assignmentIds.isEmpty {
                ...
                let progressResults = try await sharedDatabase.records(matching: progressQuery, inZoneWith: sharedZone.zoneID)
            ...
            let allProgressResults = try await sharedDatabase.records(matching: allProgressQuery, inZoneWith: sharedZone.zoneID)
        ...
        do {
            ...
            let documentResults = try await sharedDatabase.records(matching: documentQuery, inZoneWith: sharedZone.zoneID)
            ...
            if !documentRecordIDs.isEmpty {
                let deleteResults = try await sharedDatabase.modifyRecords(saving: [], deleting: documentRecordIDs)
                var deletedCount = 0
                for (_, result) in deleteResults.deleteResults {
                    if case .success = result {
                        deletedCount += 1
        ...
        do {
            let personalInfoRecordID = CKRecord.ID(recordName: "\(studentIdString)-personal-info", zoneID: sharedZone.zoneID)
        ...
        do {
            let trainingGoalsRecordID = CKRecord.ID(recordName: "\(studentIdString)-training-goals", zoneID: sharedZone.zoneID)
        ...
        do {
            ...
            let customResults = try await sharedDatabase.records(matching: customQuery, inZoneWith: sharedZone.zoneID)
            ...
            if !customRecordIDs.isEmpty {
                let deleteResults = try await sharedDatabase.modifyRecords(saving: [], deleting: customRecordIDs)
                var deletedCount = 0
                for (_, result) in deleteResults.deleteResults {
                    if case .success = result {
                        deletedCount += 1
        ...
        do {
            ...
            let endorsementQuery = CKQuery(recordType: "EndorsementImage", predicate: endorsementPredicate)
            let endorsementResults = try await sharedDatabase.records(matching: endorsementQuery, inZoneWith: sharedZone.zoneID)
            ...
            if !endorsementRecordIDs.isEmpty {
                let deleteResults = try await sharedDatabase.modifyRecords(saving: [], deleting: endorsementRecordIDs)
                var deletedCount = 0
                for (_, result) in deleteResults.deleteResults {
                    if case .success = result {
                        deletedCount += 1
    ...
    func removeShareForStudent(_ student: Student, modelContext: ModelContext) async -> Bool {
        ...
        do {
            ...
            print("✅ Share removed successfully - all shared data deleted, student app will detect this on next sync")
            return true
        } catch {
    ...
    func acceptShareFromURL(_ urlString: String) async -> (success: Bool, error: String?) {
        ...
        guard url.absoluteString.contains("icloud.com/share") else {
            let error = "This doesn't appear to be a valid CloudKit share URL. Please check the URL and try again."
            print("❌ \(error)")
            errorMessage = error
            return (false, error)
        }
        ...
        for attempt in 1...3 {
            do {
                ...
                metadataFetchError = error
                print("❌ Failed to fetch share metadata (attempt \(attempt)/3): \(error.localizedDescription)")
                if attempt < 3 {
        ...
        guard let shareMetadata = metadata else {
            if let ckError = metadataFetchError as? CKError {
                ...
                switch ckError.code {
                case .unknownItem:
                    userFriendlyError = "Share not found. The share link may be invalid, expired, or the instructor may have removed it. Please ask your instructor to send a new link."
                case .notAuthenticated:
                    userFriendlyError = "You are not signed in to iCloud. Please sign in to iCloud in Settings and try again."
                case .networkUnavailable, .networkFailure:
                    userFriendlyError = "No internet connection. Please check your network and try again."
                case .serviceUnavailable:
                    userFriendlyError = "CloudKit service is temporarily unavailable. Please try again in a few moments."
                default:
                    userFriendlyError = "Failed to fetch share information: \(ckError.localizedDescription). Please try again."
                }
                errorMessage = userFriendlyError
                return (false, userFriendlyError)
            } else {
                let error = "Failed to fetch share information: \(metadataFetchError?.localizedDescription ?? "Unknown error"). Please try again."
                errorMessage = error
                return (false, error)
            }
        }
        ...
        for attempt in 1...3 {
            do {
                ...
                switch error.code {
                case .unknownItem:
                    userFriendlyError = "Share not found. The share may have been removed or expired. Please ask your instructor to send a new link."
                case .notAuthenticated:
                    userFriendlyError = "You are not signed in to iCloud. Please sign in to iCloud in Settings and try again."
                case .quotaExceeded:
                ...
                case .participantMayNeedVerification:
                    userFriendlyError = "Your iCloud account needs verification. Please check your iCloud settings and try again."
                case .invalidArguments:
                    userFriendlyError = "Invalid share URL. Please check the URL and try again."
                case .permissionFailure:
                    userFriendlyError = "You don't have permission to accept this share. Please contact your instructor."
                default:
                    userFriendlyError = "Failed to accept share: \(error.localizedDescription). Please try again."
                }
    ...
    func checkForStudentInitiatedUnlink(for student: Student, modelContext: ModelContext) async -> Bool {
        ...
        do {
            ...
            let sharedDatabase = container.sharedCloudDatabase
            let studentRecordID = CKRecord.ID(recordName: student.id.uuidString, zoneID: sharedZone.zoneID)
    ...
    func checkAndNotifyShareAcceptance(for student: Student) async {
        ...
        do {
            ...
            if hasParticipants && !alreadyNotified {
                ...
                await PushNotificationService.shared.notifyInstructorOfShareAcceptance(studentName: student.displayName)
    ...
    func syncStudentPersonalInfo(_ student: Student, modelContext: ModelContext) async {
        ...
        do {
            ...
            let personalInfoRecord = createStudentPersonalInfoRecord(from: student)
            let recordID = CKRecord.ID(recordName: "\(student.id.uuidString)-personal-info", zoneID: sharedZone.zoneID)
    ...
    private func syncAssignmentsToSharedZone(_ student: Student, modelContext: ModelContext) async {
        do {
            ...
            for assignment in student.checklistAssignments ?? [] {
                await syncAssignmentToSharedZone(assignment, student: student, customZone: customZone, modelContext: modelContext)
            }
    ...
    @MainActor
    private func ensureAllItemProgressRecordsExist(for assignment: ChecklistAssignment, modelContext: ModelContext) {
        guard let template = assignment.template, let templateItems = template.items else {
            print("⚠️ Cannot ensure ItemProgress records: template or items are nil for assignment '\(assignment.displayName)'")
            return
        }
        ...
        if !assignment.isCustomChecklist, let templateIdentifier = assignment.templateIdentifier {
            if let library = loadStudentLibrary(),
               let libraryChecklist = library.checklists.first(where: { $0.templateIdentifier?.lowercased() == templateIdentifier.lowercased() }) {
                ...
                if templateItemCount != libraryItemCount {
                    ...
                    if libraryItemCount == 0 && templateItemCount > 0 {
                        ...
                        print("   Template has \(templateItemCount) items, library has \(libraryItemCount) items")
        ...
        if createdCount > 0 {
            print("✅ Created \(createdCount) missing ItemProgress records for assignment '\(assignment.displayName)' (now has \(assignment.itemProgress?.count ?? 0)/\(templateItems.count) items)")
            do {
                try modelContext.save()
            } catch {
            ...
            if currentCount != expectedCount {
                print("⚠️ ItemProgress count mismatch for assignment '\(assignment.displayName)': has \(currentCount), expected \(expectedCount)")
                ...
                print("   This may indicate the template is missing items or ItemProgress records were deleted")
            } else {
                ...
                if assignment.templateIdentifier == "default_bi_annual_flight_review" {
                    print("✅ ItemProgress count verified for '\(assignment.displayName)': \(currentCount) items")
                }
            }
        }
    }
    ...
    private func syncAssignmentToSharedZone(_ assignment: ChecklistAssignment, student: Student, customZone: CKRecordZone, modelContext: ModelContext, syncItemProgress: Bool = true) async {
        ...
        if let sharedZone = try? await getSharedZone(for: student) {
            ...
            print("⚠️ Shared zone not yet available - writing to private zone (will sync to shared once zone is created)")
            print("   This is normal immediately after share acceptance - CloudKit may take a few seconds to create the shared zone")
        }
        ...
        do {
            ...
            let resolvedIDs = assignmentRecordData.itemProgress.prefix(5).map { $0.templateItemId.prefix(8) }.joined(separator: ", ")
            print("📤 SYNC: Saving ChecklistAssignmentRecord with \(assignmentRecordData.itemProgress.count) ItemProgress records")
            print("   First 5 resolved templateItemIds: \(resolvedIDs)")
            
            let assignmentRecordID = CKRecord.ID(recordName: assignment.id.uuidString, zoneID: targetZone.zoneID)
            ...
            let wasComplete = (existingAssignmentRecord?["itemProgress"] as? String).flatMap { jsonString in
                guard let jsonData = jsonString.data(using: .utf8),
                      let items = try? JSONDecoder().decode([ItemProgressData].self, from: jsonData) else {
                    return false
                }
            ...
            let studentRecordID = CKRecord.ID(recordName: student.id.uuidString, zoneID: targetZone.zoneID)
            ...
            print("✅ Synced ChecklistAssignmentRecord with embedded ItemProgress: \(assignment.displayName)")
            ...
            if syncItemProgress {
                let assignmentRecordID = CKRecord.ID(recordName: assignment.id.uuidString, zoneID: targetZone.zoneID)
                await syncItemProgressToSharedZone(assignment, assignmentRecordID: assignmentRecordID, sharedZone: targetZone, sharedDatabase: targetDatabase)
            }
            ...
            if assignment.isCustomChecklist {
                ...
                await syncCustomChecklistDefinitionToSharedZone(assignment, student: student, sharedZone: targetZone, targetDatabase: targetDatabase)
            }
            ...
            if error.code == .serverRecordChanged {
                ...
                print("✅ Assignment '\(assignment.displayName)' already exists in CloudKit (updated if needed)")
            } else {
                print("⚠️ Failed to sync assignment '\(assignment.displayName)': \(error.localizedDescription)")
    ...
    private func syncCustomChecklistDefinitionToSharedZone(_ assignment: ChecklistAssignment, student: Student, sharedZone: CKRecordZone, targetDatabase: CKDatabase) async {
        ...
        do {
            ...
            let definitionRecordID = CKRecord.ID(recordName: "custom-definition-\(assignment.templateId.uuidString)", zoneID: sharedZone.zoneID)
                ...
                do {
                    ...
                    if error.code == .serverRecordChanged && retryCount < maxRetries - 1 {
                        ...
                        if let latestRecord = try? await targetDatabase.record(for: definitionRecordID) {
                            ...
                            definitionRecord.setValuesForKeys(latestRecord.allKeys().reduce(into: [String: Any]()) { result, key in
                        ...
                        do {
                            ...
                            if saveError.code == .invalidArguments || saveError.localizedDescription.contains("not deployed") {
                                ...
                                do {
                                    _ = try await targetDatabase.save(definitionRecord)
                                    print("✅ Synced CustomChecklistDefinition after explicit schema init: \(template.name)")
                                    return
    ...
    private func syncAssignmentMetadataOnly(_ assignment: ChecklistAssignment, student: Student, customZone: CKRecordZone) async {
        ...
        do {
            let assignmentRecordID = CKRecord.ID(recordName: assignment.id.uuidString, zoneID: sharedZone.zoneID)
    ...
    private func syncItemProgressForAssignment(_ assignment: ChecklistAssignment, student: Student, customZone: CKRecordZone, modelContext: ModelContext) async {
        // Use the new architecture - sync entire ChecklistAssignmentRecord with embedded ItemProgress
        await syncAssignmentToSharedZone(assignment, student: student, customZone: customZone, modelContext: modelContext, syncItemProgress: true)
    }
    ...
    func syncSingleItemProgressToSharedZone(itemProgress: ItemProgress, assignment: ChecklistAssignment, student: Student, modelContext: ModelContext) async {
        guard student.shareRecordID != nil else {
            return
        }
    ...
    private func syncItemProgressToSharedZone(_ assignment: ChecklistAssignment, assignmentRecordID: CKRecord.ID, sharedZone: CKRecordZone, sharedDatabase: CKDatabase) async {
        ...
        do {
            ...
            for item in items {
                ...
                let libraryItemID = resolveLibraryItemID(for: item, assignment: assignment) ?? item.templateItemId
                if libraryItemID != item.templateItemId && syncedCount < 3 {
                    print("🔄 Resolved library item ID: \(item.templateItemId.uuidString) -> \(libraryItemID.uuidString)")
                }
                let templateItemIdString = libraryItemID.uuidString
                guard !templateItemIdString.isEmpty else {
                    print("❌ ERROR: ItemProgress has empty templateItemId - skipping sync for item ID: \(item.id.uuidString)")
                    continue
                }
                ...
                if item.isComplete {
                    completedCount += 1
                    print("   ✅ Synced completed item: templateItemId=\(item.templateItemId.uuidString) (full UUID), isComplete=true")
                } else if syncedCount <= 3 {
                    // Log first few incomplete items for debugging
                    print("   📋 Synced incomplete item: templateItemId=\(item.templateItemId.uuidString) (full UUID), isComplete=false")
                }
            }
    ...
    private func validateItemProgressRecords(for assignment: ChecklistAssignment) async {
        ...
        for progress in itemProgress {
            ...
            if !foundInTemplate {
                ...
                validationErrors.append("ItemProgress with templateItemId \(templateItemId.uuidString) doesn't match any template item")
            }
        ...
        if progressCount != templateItemCount {
            validationErrors.append("ItemProgress count (\(progressCount)) doesn't match template items count (\(templateItemCount))")
        }
        
        // Report validation results
        if !validationErrors.isEmpty {
            print("⚠️ ItemProgress validation found \(validationErrors.count) issue(s) for assignment '\(assignment.displayName)':")
    ...
    private func saveRecordWithConflictResolution(_ record: CKRecord, database: CKDatabase? = nil) async throws -> CKRecord {
    ...
    private func intelligentMerge(ourRecord: CKRecord, serverRecord: CKRecord) -> CKRecord {
        ...
        case "ChecklistAssignment", "ItemProgress":
            ...
            print("📝 Using overwrite strategy for \(ourRecord.recordType) - instructor version always wins")
    ...
    func deleteAssignmentFromCloudKit(assignment: ChecklistAssignment, student: Student) async {
        ...
        if let itemProgress = assignment.itemProgress {
            for progress in itemProgress {
                let progressRecordID = CKRecord.ID(recordName: progress.id.uuidString, zoneID: targetZone.zoneID)
                do {
                    ...
                    if let ckError = error as? CKError, ckError.code == .unknownItem {
                        ...
                        print("⚠️ Failed to delete ItemProgress record \(progress.id.uuidString.prefix(8)): \(error)")
                    }
