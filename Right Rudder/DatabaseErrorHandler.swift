...
enum DatabaseErrorHandler {
    ...
    static func isDiskIOError(_ error: Error) -> Bool {
        ...
        if errorString.contains("disk i/o error") ||
           errorString.contains("i/o error for database") ||
    ...
    static func isCorruptionError(_ error: Error) -> Bool {
        ...
        let errorString = error.localizedDescription.lowercased()
        return errorString.contains("corruption") ||
    ...
    static func saveWithRecovery(_ context: ModelContext, maxAttempts: Int = 3) async throws -> Bool {
        ...
        guard isValid else {
            ...
            throw NSError(domain: "DatabaseErrorHandler", code: -1, userInfo: [NSLocalizedDescriptionKey: "ModelContext is invalid"])
        }
