...
struct InstructorInfoView: View {
  ...
  var body: some View {
    NavigationView {
      Form {
        Section {
          VStack(alignment: .leading, spacing: 8) {
            Text("Instructor Information")
              ...
              .fontWeight(.bold)
            Text(
              "Enter your information. This will be automatically added to all new student records when they are created."
            )
            .font(.subheadline)
        ...
        Section("Your Details") {
          ...
          VStack(alignment: .leading, spacing: 8) {
            ...
            if tempInstructorCFIHasExpiration {
              DatePicker(
                "CFI Expiration Date", selection: $tempInstructorCFIExpirationDate,
                displayedComponents: .date
              )
              .font(.caption)
            } else {
              DatePicker(
                "CFI Recent Experience Date", selection: $tempInstructorCFIExpirationDate,
                displayedComponents: .date
              )
              .font(.caption)
            }
        ...
        Section {
          Text(
            "This information will be automatically added to all new student records when you create them. Existing student records will not be affected."
          )
          .font(.caption)
          .foregroundColor(.secondary)
        }
      ...
      .alert("Information Saved", isPresented: $showingSavedAlert) {
        ...
        Text(
          "Your instructor information has been saved and will be added to all new student records."
        )
      }
      .onAppear {
        loadExistingInfo()
  ...
  private func saveInstructorInfo() {
    ...
    print(
      "Instructor information saved: \(instructorName), CFI: \(instructorCFINumber), Expiration: \(instructorCFIHasExpiration ? "Yes" : "No"), Date: \(instructorCFIExpirationDateString)"
    )
