...
struct RightRudderApp: App {
  ...
  var body: some Scene {
    WindowGroup {
      ...
      .task {
        ...
        await CloudKitShareService.shared.monitorShareAcceptanceForAllStudents(
      ...
      .alert("Database Recovery", isPresented: $databaseRecoveryService.showRecoveryAlert) {
        ...
        Text(
          "Your database appears to be corrupted. Would you like to attempt recovery? This will restore your data from CloudKit."
        )
      }
      ...
      .alert("Database Error", isPresented: $showInvalidationAlert) {
        ...
        Text(
          "The database encountered an error and some data may have been lost. For best results, please restart the app. Your data will be restored from CloudKit on next sync."
        )
      }
    }
  ...
  private static func isCoreDataCorruptionError(_ error: Error) -> Bool {
    ...
    return DatabaseErrorHandler.isCorruptionError(error)
      || DatabaseErrorHandler.isDiskIOError(error)
  }
