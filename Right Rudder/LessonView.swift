    ...
    if hasUnsavedChanges {
      ...
      for (itemId, isComplete) in itemStates {
        ...
        if progressItem == nil {
          ...
          if displayItems.contains(where: { $0.templateItemId == itemId }) {
            ...
            print(
              "⚠️ Created missing ItemProgress record for templateItemId: \(itemId) - this should not happen if assignment was created correctly"
            )
          } else {
      ...
      for displayItem in displayItems {
        ...
        if progress.itemProgress?.first(where: { $0.templateItemId == templateItemId }) == nil {
          ...
          print(
            "⚠️ Created missing ItemProgress record for templateItemId: \(templateItemId) - this should not happen if assignment was created correctly"
          )
        }
      }
      ...
      do {
        saveSuccessful = try await DatabaseErrorHandler.saveWithRecovery(
          modelContext, maxAttempts: 3)
        ...
        if DatabaseErrorHandler.isInvalidationError(error) {
          print(
            "⚠️ Model invalidation detected - data may have been lost. User should restart the app.")
      ...
      if saveSuccessful {
        ...
        print(
          "🔍 Verification: Progress shows \(completedCount)/\(totalCount) items completed after save"
        )
  ...
  private func refreshLocalData() async {
    ...
    if let refreshedProgress = try? modelContext.fetch(request).first {
      ...
      await MainActor.run {
        ...
        print(
          "✅ Local data refreshed - template: \(template?.name ?? "nil"), items: \(refreshedProgress.itemProgress?.count ?? 0)"
        )
      }
    } else {
    ...
    Group {
      if template == nil {
        VStack(spacing: 16) {
          ...
          ProgressView()
          Text(
            "If this persists, the template may not be available. Please try again or contact support."
          )
          .font(.caption)
      ...
      if template == nil {
        ...
        if let templates = try? modelContext.fetch(request) {
          ...
          if !defaultTemplatesExist || templates.isEmpty {
            print(
              "⚠️ No default templates found or templates empty. Initializing default templates...")
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
              ...
              if self.template == nil {
                ...
                if let retryTemplates = try? self.modelContext.fetch(retryRequest) {
                  print(
                    "❌ Template still not found after initialization. Available templates: \(retryTemplates.map { $0.name })"
                  )
                  ...
                  if let identifier = self.progress.templateIdentifier,
                    let foundTemplate = retryTemplates.first(where: {
                      $0.templateIdentifier == identifier
                    })
                  {
