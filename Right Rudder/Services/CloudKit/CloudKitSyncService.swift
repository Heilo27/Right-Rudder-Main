...
class CloudKitSyncService: ObservableObject {
  ...
  func initializeCloudKitSchema() async {
    ...
    do {
      ...
      do {
        ...
        } else if saveError.code == .internalError
  ...
  private func getCorrectTemplateItemID(for item: ItemProgress, assignment: ChecklistAssignment)
    -> String
  {
  ...
  private func syncStudentToCloudKit(_ student: Student) async {
    do {
      ...
      if let existing = existingRecord {
        ...
        updateIfChangedBool(
          "instrumentGroundSchoolCompleted", newValue: student.instrumentGroundSchoolCompleted)
        updateIfChangedBool(
          "instrumentWrittenTestCompleted", newValue: student.instrumentWrittenTestCompleted)

        // Training milestones - Commercial
        updateIfChangedBool(
          "commercialGroundSchoolCompleted", newValue: student.commercialGroundSchoolCompleted)
        updateIfChangedBool(
          "commercialWrittenTestCompleted", newValue: student.commercialWrittenTestCompleted)
        ...
        record["instrumentGroundSchoolCompleted"] = NSNumber(
          value: student.instrumentGroundSchoolCompleted)
        record["instrumentWrittenTestCompleted"] = NSNumber(
          value: student.instrumentWrittenTestCompleted)
        record["commercialGroundSchoolCompleted"] = NSNumber(
          value: student.commercialGroundSchoolCompleted)
        record["commercialWrittenTestCompleted"] = NSNumber(
          value: student.commercialWrittenTestCompleted)
    ...
    do {
      ...
      if existingAssignments.isEmpty {
        ...
        _ = ChecklistAssignmentService.ensureTemplateRelationship(
          for: assignment, modelContext: modelContext)
  ...
  private func restoreChecklistItems(_ assignment: ChecklistAssignment) async {
    do {
      let database = container.privateCloudDatabase
      let query = CKQuery(
        recordType: "ItemProgress",
        predicate: NSPredicate(format: "assignmentId == %@", assignment.id.uuidString))
      let results = try await database.records(matching: query)
  ...
  private func restoreChecklistItemFromRecord(_ record: CKRecord, assignment: ChecklistAssignment)
    async
  {
    guard let modelContext = modelContext else { return }
  ...
  private func restoreTemplateItems(_ template: ChecklistTemplate) async {
    do {
      ...
      let query = CKQuery(
        ...
        predicate: NSPredicate(format: "templateId == %@", template.id.uuidString))
      let results = try await database.records(matching: query)
  ...
  private func syncStudentChecklistsToSharedZone(_ student: Student) async {
    ...
    guard hasActive else {
      print(
        "⚠️ No active share (student hasn't accepted) for \(student.displayName), skipping shared zone sync"
      )
      return
    }
    ...
    do {
      ...
      let allAssignmentsFromDB = try modelContext.fetch(FetchDescriptor<ChecklistAssignment>())
      ...
      print(
        "🔄 Full sync pass: Syncing assignments to shared zone for student: \(student.displayName)")
      ...
      if allAssignmentsFromDB.count > assignmentsFromRelationship.count {
        print(
          "   ⚠️ WARNING: Relationship count (\(assignmentsFromRelationship.count)) is LESS than DB count (\(allAssignmentsFromDB.count))"
        )
      ...
      do {
        let query = CKQuery(
          recordType: "ChecklistAssignment",
          predicate: NSPredicate(format: "studentId == %@", student.id.uuidString))
      ...
      for (index, assignment) in assignments.enumerated() {
        do {
          ...
          if index == 0 || (index + 1) % 10 == 0 || index == assignments.count - 1 {
            print(
              "   ✅ Synced \(syncedCount)/\(assignments.count) assignments: \(assignment.displayName)"
            )
          }
        } catch {
          errorCount += 1
          print(
            "   ❌ Failed to sync assignment \(assignment.displayName) (ID: \(assignment.id.uuidString)): \(error)"
          )
        }
      }
      ...
      do {
        ...
        print(
          "   📊 Synced: \(syncedCount) successful, \(errorCount) failed out of \(assignments.count) total assignments"
        )
        print("   🔍 Final count in shared zone: \(finalCount) assignment records")
        if finalCount < assignments.count {
          print(
            "   ⚠️ WARNING: Only \(finalCount) assignments found in shared zone, expected \(assignments.count)!"
          )
        }
      } catch {
        print("   ⚠️ Could not verify final count: \(error)")
        print(
          "✅ Full sync pass completed: \(syncedCount) successful, \(errorCount) failed out of \(assignments.count) total assignments"
        )
      }
    } catch {
  ...
  private func syncChecklistToSharedZone(
    ...
    do {
      ...
      if hasChanges {
        _ = try await saveRecordWithConflictResolution(
          record, database: container.privateCloudDatabase)
  ...
  private func syncChecklistItemsToSharedZone(
    _ assignment: ChecklistAssignment, customZone: CKRecordZone
  ) async {
    do {
      ...
      for item in items {
        ...
        let needsSync =
          existingRecord == nil || cloudKitLastModified == nil
          || localLastModified > cloudKitLastModified!
      ...
      if itemsWithProgress > 0 {
        print(
          "      📊 Synced \(itemsWithProgress) items with progress changes, \(itemsSynced) total items"
        )
      }
    } catch {
  ...
  private func syncStudentChangesFromSharedZone(_ zone: CKRecordZone, modelContext: ModelContext)
    async
  {
    do {
      let sharedDatabase = container.sharedCloudDatabase
  ...
  private func updateStudentFromSharedRecord(_ record: CKRecord, modelContext: ModelContext) async {
    do {
      ...
      // Training milestones - PPL
      student.pplGroundSchoolCompleted =
        record["pplGroundSchoolCompleted"] as? Bool ?? student.pplGroundSchoolCompleted
      student.pplWrittenTestCompleted =
        record["pplWrittenTestCompleted"] as? Bool ?? student.pplWrittenTestCompleted

      // Training milestones - Instrument
      student.instrumentGroundSchoolCompleted =
        record["instrumentGroundSchoolCompleted"] as? Bool
        ?? student.instrumentGroundSchoolCompleted
      student.instrumentWrittenTestCompleted =
        record["instrumentWrittenTestCompleted"] as? Bool ?? student.instrumentWrittenTestCompleted

      // Training milestones - Commercial
      student.commercialGroundSchoolCompleted =
        record["commercialGroundSchoolCompleted"] as? Bool
        ?? student.commercialGroundSchoolCompleted
      student.commercialWrittenTestCompleted =
        record["commercialWrittenTestCompleted"] as? Bool ?? student.commercialWrittenTestCompleted

      // Training milestones - CFI
      student.cfiGroundSchoolCompleted =
        record["cfiGroundSchoolCompleted"] as? Bool ?? student.cfiGroundSchoolCompleted
      student.cfiWrittenTestCompleted =
        record["cfiWrittenTestCompleted"] as? Bool ?? student.cfiWrittenTestCompleted
  ...
  private func syncStudentDocumentsFromSharedZone(_ zone: CKRecordZone, modelContext: ModelContext)
    async
  {
    do {
      let sharedDatabase = container.sharedCloudDatabase
  ...
  private func updateStudentDocumentFromSharedRecord(_ record: CKRecord, modelContext: ModelContext)
    async
  {
    do {
      ...
      if let existingDocument = existingDocuments.first {
        ...
        if sharedLastModified > existingDocument.lastModified {
          ...
          existingDocument.documentTypeRaw =
            record["documentType"] as? String ?? existingDocument.documentTypeRaw
        ...
        print(
          "➕ Creating new document from shared zone: \(record["filename"] as? String ?? "Unknown")")

        let documentType =
          DocumentType(rawValue: record["documentType"] as? String ?? "")
  ...
  private func saveRecordWithConflictResolution(_ record: CKRecord, database: CKDatabase)
