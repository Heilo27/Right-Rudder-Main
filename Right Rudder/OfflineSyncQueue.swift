...
class OfflineSyncManager: ObservableObject {
  ...
  private func processOperation(_ operation: OfflineSyncOperation) async {
    ...
    do {
      ...
      if success {
        ...
        print(
          "Failed to process operation: \(operation.operationType), retry count: \(operation.retryCount)"
        )
      }
  ...
  private func executeChecklistUpdate(_ operation: OfflineSyncOperation) async -> Bool {
    guard let modelContext = modelContext,
      let checklistId = operation.checklistId
    else { return false }
    ...
    do {
      ...
      guard let foundChecklist = checklist,
        let student = foundChecklist.student
      else { return false }

      // Sync to CloudKit
      let shareService = CloudKitShareService.shared
      await shareService.syncStudentChecklistAssignmentsToSharedZone(
  ...
  private func executeChecklistAdd(_ operation: OfflineSyncOperation) async -> Bool {
    ...
    do {
      ...
      let shareService = CloudKitShareService.shared
      await shareService.syncStudentChecklistAssignmentsToSharedZone(
        foundStudent, modelContext: modelContext)
