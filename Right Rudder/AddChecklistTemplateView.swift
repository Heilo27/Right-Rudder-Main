      ...
      Form {
        ...
        Section(header: Text("Relevant Data (Optional)")) {
          TextEditor(text: $relevantData)
            ...
            .modifier(ResponsiveTextFieldModifier())
          Text(
            "Add study materials, ACS references, or other lesson-specific information. Leave blank if not applicable."
          )
          .font(.caption)
    ...
    AddChecklistTemplateView(category: "PPL")
      .modelContainer(
        for: [
          ...
          CustomChecklistItem.self, EndorsementImage.self, ChecklistTemplate.self,
          ChecklistItem.self,
        ], inMemory: true)
  }
}
